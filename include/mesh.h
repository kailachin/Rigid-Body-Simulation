...
public:
  ...
  Matrix3d get_curr_inv_IT(){
    ...
    //rotate the inverse intertia tensor
    Matrix3d currInvIT = R * invIT * R.transpose();
  ...
  void update_position(double timeStep){
    ...
    //angular velocity / position update

    RowVector4d rotQuat;
    rotQuat << 0.0, 0.5* angVelocity * timeStep;
    //RowVector4d RotQuat = QExp(rotQuat);
  ...
  void update_velocity(double timeStep){
    ...
    RowVector3d gravity(0, -9.8, 0);  //constant for gravity
